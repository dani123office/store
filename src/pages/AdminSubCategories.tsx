...
const AdminSubCategories = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await customFetch.delete(`/sub-categories/${deleteTarget.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await customFetch.put(`/sub-categories/${editing.id}`, form);
  ...
  return (
    <div>
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1">
                    <input type="text" value={form.subcat_img}
                      onChange={(e) => setForm({ ...form, subcat_img: e.target.value })}
      ...
      <div className="bg-white border border-[#e0e0e0] rounded-lg overflow-x-auto">
            ...
            {filtered.map((item) => (
              <tr key={item.id} className="border-t border-[#e0e0e0] hover:bg-[#fafafa]">
                <td className="py-2 px-5">
                  {item.subcat_img ? (
                    <img src={`/assets/${item.subcat_img}`} alt={item.subcat_title} className="w-10 h-10 object-cover rounded" />
                  ) : (
                    <div className="w-10 h-10 bg-[#f1f1f1] rounded" />
                  )}
                </td>
                <td className="py-3 px-5 font-medium text-[#202223]">{item.subcat_title}</td>
                <td className="py-3 px-5 text-[#6d7175]">{getCategoryTitle(item.cat_id)}</td>
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-[#6d7175] p-6 text-center">No subcategories found.</p>
        )}
      </div>
